...
from app.config import MAX_CONCURRENT_REQUESTS, REQUEST_TIMEOUT, HTTP_VERIFY
from app.core.http_client import create_async_client, fetch_with_retry, get_request_headers
...
async def check_single_site(client: httpx.AsyncClient, site: Dict[str, Any], username: str, seed_name: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        if special_handler == "instagram":
            ...
            resp = await fetch_with_retry(client, insta_url, follow_redirects=True)
        ...
        # 2. FACEBOOK PROBE
            ...
            resp = await fetch_with_retry(client, fb_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200:
                text = resp.text
        ...
        # 3. TIKTOK PROBE
            ...
            resp = await fetch_with_retry(client, tt_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200:
                data = resp.json()
        ...
        # 4. HACKERNEWS (Firebase JSON API)
        elif special_handler == "hackernews":
            hn_api = f"https://hacker-news.firebaseio.com/v0/user/{u_clean}.json"
            resp = await fetch_with_retry(client, hn_api, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200 and resp.text and resp.text.strip() != "null":
                ...
                if data and data.get("id"):
                    ...
                    clean_about = html.unescape(re.sub(r'<[^>]+>', ' ', about)).strip()
                    result["found"] = True
                    result["metadata"] = {
                        "display_name": data.get("id"),
                        "bio": clean_about or f"HackerNews User (Karma: {karma})",
                        "avatar_url": None,
                        "avatar_hash": None,
                        "metrics": {"karma": str(karma)}
                    }

        # 5. SPOTIFY PROBE
        elif special_handler == "spotify":
            sp_url = f"https://open.spotify.com/user/{u_clean}"
            resp = await fetch_with_retry(client, sp_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200:
        ...
        # 6. TWITCH PROBE
            ...
            resp = await fetch_with_retry(client, tw_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200:
                m_title = re.search(r'<meta\s+property=["\']og:title["\']\s+content=["\'](.*?)["\']', resp.text, re.I)
                m_desc = re.search(r'<meta\s+property=["\']og:description["\']\s+content=["\'](.*?)["\']', resp.text, re.I)
        ...
        # 7. VERO PROBE
        elif special_handler == "vero":
            v_url = f"https://vero.co/{u_clean}"
            resp = await fetch_with_retry(client, v_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200:
                m_og_title = re.search(r'<meta\s+property=["\']og:title["\']\s+content=["\'](.*?)["\']', resp.text, re.I)
                m_og_img = re.search(r'<meta\s+property=["\']og:image["\']\s+content=["\'](.*?)["\']', resp.text, re.I)
                m_desc = re.search(r'<meta\s+property=["\']og:description["\']\s+content=["\'](.*?)["\']', resp.text, re.I)
                if m_og_title:
                    v_title = html.unescape(m_og_title.group(1)).strip()
                    if v_title and ("vero" not in v_title.lower() or "on vero" in v_title.lower()):
                        img_url = m_og_img.group(1).strip() if m_og_img else None
                        result["found"] = True
                        result["metadata"] = {
                            "display_name": v_title.replace(" on VERO", "").strip(),
                            "bio": m_desc.group(1) if m_desc else "",
                            "avatar_url": img_url,
                            "avatar_hash": generate_avatar_fingerprint(img_url),
                            "metrics": {}
        ...
        # 8. WATTPAD PROBE
        elif special_handler == "wattpad":
            wp_url = f"https://www.wattpad.com/user/{u_clean}"
            resp = await fetch_with_retry(client, wp_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200:
                m_title = re.search(r'<title>(.*?)</title>', resp.text, re.I)
                title_t = html.unescape(m_title.group(1)).strip() if m_title else ""
                # Non-existent Wattpad pages return generic "Wattpad - Stories to obsess over"
                if title_t and "stories to obsess over" not in title_t.lower() and "404" not in title_t.lower():
                    result["found"] = True
        ...
        # 9. GITHUB API PROBE
            ...
            resp = await fetch_with_retry(client, gh_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200:
                data = resp.json()
        ...
        # 10. REDDIT PROBE
            ...
            resp = await fetch_with_retry(client, r_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            if resp.status_code == 200:
                result["found"] = True
                result["metadata"] = extract_html_metadata(resp.text)
        ...
        # 11. TELEGRAM PROBE
            ...
            resp = await fetch_with_retry(client, tg_url, follow_redirects=True)
            result["status_code"] = resp.status_code
            text = resp.text
        ...
        # 12. STEAM XML PROBE
            ...
            resp = await fetch_with_retry(client, steam_url, follow_redirects=True)
            result["status_code"] = resp.status_code
        ...
        # 13. STANDARD MULTI-FACTOR VERIFICATION
        else:
            resp = await fetch_with_retry(client, url, follow_redirects=True)
            result["status_code"] = resp.status_code
