...
# Single shared connection, opened once and reused for the app's lifetime,
# instead of opening (and setting WAL/pragma options on) a brand-new SQLite
# connection on every single repo call -- including save_scan_result(),
# which used to run once per FOUND profile in a scan. Safe here because this
# whole app runs single-process/single-thread-for-DB-access (every repo call
# happens directly on the asyncio event loop thread, never in a worker
# thread), so there's no real concurrent-connection hazard to guard against
# beyond the double-checked lock below for the one-time creation itself.
